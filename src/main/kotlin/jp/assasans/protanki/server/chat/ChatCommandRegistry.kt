...
class ChatCommandRegistry : IChatCommandRegistry, KoinComponent {
  ...
  override suspend fun callCommand(socket: UserSocket, parsed: ParsedCommand, source: CommandInvocationSource) {
    ...
    // ChatModeratorLevel check
    val user = socket.user ?: throw IllegalStateException("User must be logged in to execute commands.")
    if (user.moderatorLevel < parsed.command.requiredLevel) {
        val context = CommandContext(socket, parsed.command, parsed.arguments, source)
        context.reply("You do not have the required permissions to use this command.", warning = true)
        return
    }
...
class Command(val name: String) {
  ...
  var requiredLevel: ChatModeratorLevel = ChatModeratorLevel.None // Added required level for commands
